...
class QuantumCircuit():
    ...
    def circuitMatrix(self):
        ...
        for i in range(0, self.noqubits - 1, 1):
            ...
            if(i == 0):
                ...
                aux = np.kron(b, aux)
        self.qubit = aux.reshape(self.nstates, 1)
        print(self.qubit)
        return 0
    ...
    #         sys.exit(1)
    #     else:
    #         x = self.objs[control].read()
    #         if (x[0] == 1):
    #             aux_ctrl = self.objs[control].readraw(1)
    #             aux_tgt = self.objs[tgt].readraw(1)
    #             self.objs[control].writeraw(1, aux_tgt)
    #             self.objs[tgt].writeraw(1, aux_ctrl)
    

    # def swap(self, tgt1, tgt2):
    #     target1 = self.objs[tgt1]
    #     target2 = self.objs[tgt2]
    #     self.objs[tgt1] = target2
    #     self.objs[tgt2] = target1

    # def toffoli(self, tgt, c1, c2):
    #     self.had(tgt)
    #     self.cx(tgt, c1)
    #     self.tdg(tgt)
    #     self.cx(tgt, c2)
    ...
    def t(self, numb):
        if(numb > self.noqubits - 1):
            ...
            sys.exit(1)
        self.objs[numb].t()
        print("T gate applied on qubit ", numb)
    ...
    def tdg(self, numb):
        if(numb > self.noqubits - 1):
            ...
            sys.exit(1)
        self.objs[numb].tdg()
        print("TDG gate applied on qubit ", numb)
    ...
    # Set a qubit state as 0 or 1
    def init(self, numb, state):
        self.objs[numb].init(state)
